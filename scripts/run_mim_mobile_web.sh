...
echo "[mim-mobile-web] Starting current-source MIM on ${HOST}:${PORT} with ${WORKERS} worker(s)"
if [[ -n "${LAN_IP}" ]]; then
...
exec "${PYTHON_BIN}" -m uvicorn core.app:app --lifespan off --host "${HOST}" --port "${PORT}" --workers "${WORKERS}"
