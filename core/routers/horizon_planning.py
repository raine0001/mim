...
async def create_horizon_plan_endpoint(
    ...
    await write_journal(
        ...
        metadata_json={
            ...
            "boundary_profile": (plan.metadata_json if isinstance(plan.metadata_json, dict) else {}).get("boundary_profile", {}),
            "decision_basis": (plan.metadata_json if isinstance(plan.metadata_json, dict) else {}).get("decision_basis", {}),
            "allowed_actions": (plan.metadata_json if isinstance(plan.metadata_json, dict) else {}).get("allowed_actions", []),
            "approval_required": bool((plan.metadata_json if isinstance(plan.metadata_json, dict) else {}).get("approval_required", False)),
            "retry_policy": (plan.metadata_json if isinstance(plan.metadata_json, dict) else {}).get("retry_policy", {}),
            "risk_level": str((plan.metadata_json if isinstance(plan.metadata_json, dict) else {}).get("risk_level") or "").strip(),
