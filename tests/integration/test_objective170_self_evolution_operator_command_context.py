...
class Objective170SelfEvolutionOperatorCommandContextTest(unittest.TestCase):
    def _create_strategy(self, *, run_id: str, zone_suffix: str) -> int:
        status, payload = post_json(
            "/planning/strategies/generate",
            {
                "actor": "objective170-test",
                "source": "objective170-focused",
                "observed_conditions": [
                    {
                        "condition_type": "routine_zone_pattern",
                        "target_scope": f"front-left-obj170-{run_id}-{zone_suffix}",
                        "severity": 0.89,
                        "occurrence_count": 2,
    ...
    def _deactivate_strategy(self, *, strategy_id: int, run_id: str) -> None:
        status, payload = post_json(
            ...
            {
                "actor": "objective170-test",
                "reason": "objective170 synthetic stall pattern",
                "metadata_json": {"run_id": run_id},
            },
        )
        self.assertEqual(status, 200, payload)

    def test_objective170_self_evolution_command_context_mirrors_primary_command(self) -> None:
        run_id = uuid4().hex[:8]
        ...
        self.assertIn("self_evolution_operator_command_context", runtime_features)
        ...
        self.assertTrue(primary, self_evolution)

        conversation_context = ui_state.get("conversation_context", {}) if isinstance(ui_state, dict) else {}
        self.assertEqual(
            str(conversation_context.get("self_evolution_operator_command_method", "")).strip(),
            str(primary.get("method", "")).strip(),
            ui_state,
        )
        self.assertEqual(
            str(conversation_context.get("self_evolution_operator_command_path", "")).strip(),
            str(primary.get("path", "")).strip(),
            ui_state,
        )
        self.assertEqual(
            str(conversation_context.get("self_evolution_operator_command_purpose", "")).strip(),
            str(primary.get("purpose", "")).strip(),
            ui_state,
        )
        self.assertTrue(
            str(conversation_context.get("self_evolution_operator_command_path", "")).startswith("/improvement/"),
            ui_state,
        )
