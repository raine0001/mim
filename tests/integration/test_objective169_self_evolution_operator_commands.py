...
class Objective169SelfEvolutionOperatorCommandsTest(unittest.TestCase):
    def _create_strategy(self, *, run_id: str, zone_suffix: str) -> int:
        status, payload = post_json(
            "/planning/strategies/generate",
            {
                "actor": "objective169-test",
                "source": "objective169-focused",
                "observed_conditions": [
                    {
                        "condition_type": "routine_zone_pattern",
                        "target_scope": f"front-left-obj169-{run_id}-{zone_suffix}",
                        "severity": 0.88,
                        "occurrence_count": 2,
    ...
    def _deactivate_strategy(self, *, strategy_id: int, run_id: str) -> None:
        status, payload = post_json(
            ...
            {
                "actor": "objective169-test",
                "reason": "objective169 synthetic stall pattern",
                "metadata_json": {"run_id": run_id},
            },
        )
        self.assertEqual(status, 200, payload)

    def test_objective169_self_evolution_operator_commands_are_visible(self) -> None:
        run_id = uuid4().hex[:8]
        ...
        self.assertIn("self_evolution_operator_commands", runtime_features)
        ...
        operator_commands = self_evolution.get("operator_commands", []) if isinstance(self_evolution.get("operator_commands", []), list) else []
        self.assertGreaterEqual(len(operator_commands), 1, self_evolution)

        primary = self_evolution.get("primary_operator_command", {}) if isinstance(self_evolution.get("primary_operator_command", {}), dict) else {}
        self.assertEqual(primary, operator_commands[0], self_evolution)
        self.assertIn(str(primary.get("method", "")).strip(), {"GET", "POST"})
        self.assertTrue(str(primary.get("path", "")).startswith("/improvement/"), primary)
        self.assertTrue(str(primary.get("purpose", "")).strip(), primary)

        self.assertTrue(str(self_evolution.get("operator_command_summary", "")).strip(), self_evolution)
        self.assertIn(str(primary.get("path", "")).strip(), str(self_evolution.get("operator_command_summary", "")).strip())

        conversation_context = ui_state.get("conversation_context", {}) if isinstance(ui_state, dict) else {}
        self.assertEqual(
            str(conversation_context.get("self_evolution_operator_command_summary", "")).strip(),
            str(self_evolution.get("operator_command_summary", "")).strip(),
            ui_state,
        )
